# -*- coding: utf-8 -*-
"""
Book & Quality v2 - Build MA Phase & Quality Labels M30 v1

Location:
    Book_Quality/02_labels/ma_quality/01_build_ma_quality_labels_m30_v1.py

Purpose:
    Build labels for the MA Phase & Quality Model with M30 anchor.

Core rule:
    Quality predicts. Candle Book confirms later.
...
TEST ONLY:
    ...
    python -u 01_build_ma_quality_labels_m30_v1.py --start "2010-03-01 00:00:00" --limit 1000 --reset

FULL BUILD:
    python -u 01_build_ma_quality_labels_m30_v1.py --reset
"""
...
SCRIPT_NAME = "01_build_ma_quality_labels_m30_v1.py"
...
DEFAULT_CONFIG = {
    ...
    "ma_quality_label_v1": {
        ...
        "min_history_bars": {"m30": 120, "h1": 120, "h4": 120},
    },
}
...
def load_config() -> Dict[str, Any]:
    ...
    # Force M30 target for this script even if older config still has M15.
...
def epoch_to_dt(ep: int) -> datetime:
...
def phase_label(current_direction: str, future_direction: str, closes: np.ndarray, highs: np.ndarray, lows: np.ndarray, end_now: int, end_future: int) -> str:
    ...
    if current_direction == "SELL" and future_direction == "BUY":
        return "REVERSAL_UP_START" if abs(slope_r) < 0.15 or decel > 0 else "DOWN_END"

    if current_direction == "BUY" and future_direction == "SELL":
        return "REVERSAL_DOWN_START" if abs(slope_r) < 0.15 or decel < 0 else "UP_END"
...
def analyze_tf(tf_data: CandleArrays, decision_epoch: int, horizon_bars: int, threshold_r: float) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
...
def first_hit_buy(highs, lows, entry, target, stop):
    tp_idx = None
    sl_idx = None
    ambiguous = False
...
def first_hit_sell(highs, lows, entry, target, stop):
    tp_idx = None
    sl_idx = None
    ambiguous = False
    for i in range(len(highs)):
...
def trade_quality(side: str, entry: float, highs, lows, closes, base_r: float) -> Dict[str, Any]:
    if len(closes) == 0:
        return {"quality_score": 0, "risk_mode": "BLOCK", "lot_multiplier": 0.0}

    if side == "BUY":
        mfe = float(np.max(highs) - entry)
        mae = float(entry - np.min(lows))
        tp1, sl1, amb = first_hit_buy(highs, lows, entry, entry + base_r, entry - base_r)
        tp2, _, _ = first_hit_buy(highs, lows, entry, entry + 2 * base_r, entry - base_r)
        tp3, _, _ = first_hit_buy(highs, lows, entry, entry + 3 * base_r, entry - base_r)
        final_move = float(closes[-1] - entry)
    else:
        mfe = float(entry - np.min(lows))
        mae = float(np.max(highs) - entry)
        tp1, sl1, amb = first_hit_sell(highs, lows, entry, entry - base_r, entry + base_r)
        tp2, _, _ = first_hit_sell(highs, lows, entry, entry - 2 * base_r, entry + base_r)
        tp3, _, _ = first_hit_sell(highs, lows, entry, entry - 3 * base_r, entry + base_r)
...
def overall_direction(tf_labels: Dict[str, Dict[str, Any]], buy_q: int, sell_q: int) -> Tuple[str, float, str]:
    weights = {"m30": 0.45, "h1": 0.35, "h4": 0.20}
    buy_vote = 0.0
    sell_vote = 0.0
    range_vote = 0.0
    ...
    reason = f"buy_vote={cn(buy_vote,3)} sell_vote={cn(sell_vote,3)} range_vote={cn(range_vote,3)}"

    if buy_p >= sell_p and buy_p >= range_p and buy_p >= 0.42:
        return "BUY", cn(buy_p * 100, 2), reason
    if sell_p >= buy_p and sell_p >= range_p and sell_p >= 0.42:
        return "SELL", cn(sell_p * 100, 2), reason
    return "RANGE", cn(max(buy_p, sell_p, range_p) * 100, 2), reason
...
def build_txt_report(report: Dict[str, Any]) -> str:
    lines = []
    lines.append("Book & Quality v2 - MA Quality Labels M30 v1 Report")
    lines.append("=" * 74)
...
def save_reports(report: Dict[str, Any], rs: str) -> None:
    rd = reports_dir()
    jp = rd / f"ma_quality_labels_m30_v1_report_{rs}.json"
    tp = rd / f"ma_quality_labels_m30_v1_report_{rs}.txt"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build bq2 MA Quality Labels M30 v1.")
...
def main() -> None:
    ...
    target_coll = bq2.get("labels_ma_quality", "bq2_labels_ma_quality_m30_v1")

    base_r = float(label_cfg.get("base_r_usd", 2.0))
    trade_horizon_m30 = int(label_cfg.get("trade_horizon_m30", 16))
    ...
    report: Dict[str, Any] = {
        ...
        "run_id": f"ma_quality_labels_m30_v1_{rs}",
        "status": "running",
        "database": db_name,
        "symbol": symbol,
        ...
        "label_config": {
            ...
            "base_r_usd": base_r,
            "trade_horizon_m30": trade_horizon_m30,
            "ma_periods": [20, 50, 100],
    ...
    print("=== Book & Quality v2 - Build MA Quality Labels M30 v1 ===", flush=True)
    ...
    try:
        ...
        ops: List[UpdateOne] = []
        max_i = data["m30"].count - 1

        for i in range(max_i):
            try:
                anchor_epoch = int(data["m30"].times[i])
                entry_idx = i + 1
                ...
                anchor_time = epoch_to_dt(anchor_epoch)
                decision_epoch = int(data["m30"].times[entry_idx])
                decision_time = epoch_to_dt(decision_epoch)
                entry_price = float(data["m30"].opens[entry_idx])
                ...
                trade_end = trade_start + trade_horizon_m30
                if trade_end > data["m30"].count:
                ...
                future_highs = data["m30"].highs[trade_start:trade_end]
                future_lows = data["m30"].lows[trade_start:trade_end]
                future_closes = data["m30"].closes[trade_start:trade_end]
        ...
        if report["counts"]["anchors_seen"] > 0 and report["counts"]["anchors_processed"] == 0 and report["counts"]["skipped_no_ma_context"] > 0:
            report["warnings"].append(
                ...
                "Run the test with a later --start date, for example: "
                "--start '2010-03-01 00:00:00'."
            )
