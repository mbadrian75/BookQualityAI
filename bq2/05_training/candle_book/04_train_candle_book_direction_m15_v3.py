# -*- coding: utf-8 -*-
"""
Book & Quality v2 - Train Candle Book Direction Model M15 v3

Input : bq2_dataset_candle_book_direction_m15_v3
Output: 06_models/candle_book/candle_book_direction_model_m15_v3_latest.joblib
Target: book_direction = BUY / SELL / UNCLEAR

TEST:
    cd C:/Project/BookQuality/bq2/05_training/candle_book
    python -u 04_train_candle_book_direction_m15_v3.py --limit-per-split 5000 --max-iter 50

FULL:
    python -u 04_train_candle_book_direction_m15_v3.py --max-iter 200 --l2-regularization 0.1
"""
...
MODEL_VERSION='candle_book_direction_model_m15_v3'
...
def models_dir():
    p=project_root()/'06_models'/'candle_book'; p.mkdir(parents=True, exist_ok=True); return p
def exports_dir():
...
def latest_feature_names() -> Tuple[List[str], Optional[str]]:
    files=sorted(exports_dir().glob('candle_book_direction_m15_v3_feature_names_*.json'))
    if not files: return [], None
    path=files[-1]
    data=read_json(path) or {}
    return list(data.get('feature_names', [])), str(path)

def load_split(db:Database, coll:str, split:str, limit:Optional[int]):
    query={'dataset_version':DATASET_VERSION,'split':split}
    cursor=db[coll].find(query, projection={'_id':0,'x':1,'y.book_direction':1}).sort('anchor_time', ASCENDING)
...
def pred_dist(labels): return dict(Counter([str(x) for x in labels]))
def label_dist(labels): return dict(Counter([str(x) for x in labels]))
...
def analyze_probabilities(y_true_labels, pred_labels, proba, classes):
    ...
    p_unclear=proba[:, idx['UNCLEAR']] if 'UNCLEAR' in idx else np.zeros(len(proba))
    conf=np.max(proba, axis=1)
    gap=np.abs(p_buy-p_sell)
    true=np.asarray(y_true_labels)
    pred=np.asarray(pred_labels)
    out={
        'avg_confidence': round(float(np.mean(conf)),6) if len(conf) else None,
        'p60_rate': round(float(np.mean(conf>=0.60)),6) if len(conf) else None,
        'p70_rate': round(float(np.mean(conf>=0.70)),6) if len(conf) else None,
        'p80_rate': round(float(np.mean(conf>=0.80)),6) if len(conf) else None,
        'avg_buy_sell_gap': round(float(np.mean(gap)),6) if len(gap) else None,
        'gap10_rate': round(float(np.mean(gap>=0.10)),6) if len(gap) else None,
        'gap20_rate': round(float(np.mean(gap>=0.20)),6) if len(gap) else None,
        'gap30_rate': round(float(np.mean(gap>=0.30)),6) if len(gap) else None,
        'gap_threshold_analysis': {},
    }
    for th in [0.05,0.10,0.15,0.20,0.25,0.30,0.40]:
        mask=(gap>=th) & (pred!='UNCLEAR')
        rows=int(mask.sum())
        out['gap_threshold_analysis'][f'gap>={th:.2f}']={
            'coverage': round(float(rows/len(gap)),6) if len(gap) else None,
            'rows': rows,
            'actual_direction_match_rate': round(float(np.mean(pred[mask]==true[mask])),6) if rows else None,
            'pred_buy_rows': int(np.sum(mask & (pred=='BUY'))),
            'pred_sell_rows': int(np.sum(mask & (pred=='SELL'))),
        }
    return out
...
def metrics_for_split(model, encoder, X, y, deps):
    ...
    pred_enc=model.predict(Xn)
    pred_labels=list(encoder.inverse_transform(pred_enc))
    proba=model.predict_proba(Xn)
    classes=list(encoder.classes_)
    out={
        'rows': len(y),
        'accuracy': round(float(accuracy_score(y_enc, pred_enc)),6),
        'macro_f1': round(float(f1_score(y_enc, pred_enc, average='macro')),6),
        'weighted_f1': round(float(f1_score(y_enc, pred_enc, average='weighted')),6),
        'log_loss': safe_log_loss(y_enc, proba, labels=list(range(len(classes)))),
        'label_distribution': label_dist(y),
        'prediction_distribution': pred_dist(pred_labels),
        'classes': classes,
        'confusion_matrix_labels': classes,
        'confusion_matrix': confusion_matrix(y_enc, pred_enc, labels=list(range(len(classes)))).tolist(),
    }
    out.update(analyze_probabilities(y, pred_labels, proba, classes))
    return out

def save_report(report, rs):
    jp=reports_dir()/f'candle_book_direction_train_m15_v3_report_{rs}.json'
    tp=reports_dir()/f'candle_book_direction_train_m15_v3_report_{rs}.txt'
    ...
    lines=['Book & Quality v2 - Train Candle Book Direction Model M15 v3 Report','='*74]
    for k in ['run_id','status','database','symbol','start_time','end_time','duration_seconds']:
        lines.append(f'{k:<34}: {report.get(k)}')
    for section,key in [('Collections','collections'),('Training Config','training_config'),('Counts','counts'),('Model Output','model_output')]:
        lines+=['',section,'-'*74]
        for k,v in report[key].items(): lines.append(f'{k:<34}: {v}')
    lines+=['','Metrics','-'*74]
    for split,m in report['metrics'].items():
...
def parse_args():
    p=argparse.ArgumentParser(description='Train Candle Book Direction M15 v3.')
    p.add_argument('--limit-per-split', type=int, default=None)
    p.add_argument('--max-iter', type=int, default=200)
    p.add_argument('--learning-rate', type=float, default=0.05)
    p.add_argument('--l2-regularization', type=float, default=0.1)
...
def main():
    ...
    report={'run_id':f'candle_book_direction_train_m15_v3_{rs}','status':'running','database':None,'symbol':None,'start_time':started.isoformat(),'end_time':None,'duration_seconds':None,'collections':{},'training_config':{},'counts':{},'model_output':{},'metrics':{},'errors':[],'args':vars(args)}
    print('=== Book & Quality v2 - Train Candle Book Direction Model M15 v3 ===', flush=True)
    try:
        ...
        cfg=load_config(); db=connect(cfg); coll=cfg['bq2_collections']['dataset_candle_book_direction']
        report['database']=cfg['database']; report['symbol']=cfg['symbol']; report['collections']={'dataset_source':coll}
        report['training_config']={'model_version':MODEL_VERSION,'dataset_version':DATASET_VERSION,'feature_version':FEATURE_VERSION,'label_version':LABEL_VERSION,'target':TARGET,'classifier':'HistGradientBoostingClassifier','max_iter':args.max_iter,'learning_rate':args.learning_rate,'l2_regularization':args.l2_regularization,'max_leaf_nodes':args.max_leaf_nodes,'limit_per_split':args.limit_per_split,'sample_weight':not args.no_sample_weight}
        X_train,y_train=load_split(db,coll,'train',args.limit_per_split)
        X_valid,y_valid=load_split(db,coll,'valid',args.limit_per_split)
        X_test,y_test=load_split(db,coll,'test',args.limit_per_split)
        if not X_train or not X_valid or not X_test: raise RuntimeError('Missing train/valid/test rows.')
        feature_count=len(X_train[0])
        report['counts']={'train':len(X_train),'valid':len(X_valid),'test':len(X_test),'rows_loaded':len(X_train)+len(X_valid)+len(X_test),'feature_count':feature_count,'train_label_distribution':label_dist(y_train),'valid_label_distribution':label_dist(y_valid),'test_label_distribution':label_dist(y_test)}
        encoder=LabelEncoder(); encoder.fit(['BUY','SELL','UNCLEAR'])
        y_train_enc=encoder.transform(y_train)
        X_train_np=np.asarray(X_train, dtype=np.float32)
        sw=make_sample_weights(y_train) if not args.no_sample_weight else None
        ...
        for split,X,y in [('train',X_train,y_train),('valid',X_valid,y_valid),('test',X_test,y_test)]:
            report['metrics'][split]=metrics_for_split(model, encoder, X, y, deps)
        ...
        bundle={'model_version':MODEL_VERSION,'dataset_version':DATASET_VERSION,'feature_version':FEATURE_VERSION,'label_version':LABEL_VERSION,'target':TARGET,'trained_at':utc_now().isoformat(),'model':model,'encoder':encoder,'classes':list(encoder.classes_),'feature_count':feature_count,'feature_names':feature_names,'feature_names_source':feature_names_source,'training_config':report['training_config'],'counts':report['counts'],'metrics':report['metrics']}
