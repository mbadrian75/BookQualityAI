# -*- coding: utf-8 -*-
"""
Book & Quality v2 - Build Candle Book Dataset M15 v1

Input : bq2_features_candle_book_m15_v1
Output: bq2_dataset_candle_book_m15_v1

TEST:
    cd C:/Project/BookQuality/bq2/04_dataset/candle_book
    python -u 01_build_candle_book_dataset_m15_v1.py --limit 1000 --reset

FULL:
    python -u 01_build_candle_book_dataset_m15_v1.py --reset
"""
...
SCRIPT_NAME = "01_build_candle_book_dataset_m15_v1.py"
...
def load_config() -> Dict[str, Any]:
    ...
    cfg["bq2_collections"]["features_candle_book"] = "bq2_features_candle_book_m15_v1"
...
def save_reports(report: Dict[str, Any], rs: str) -> None:
    jp = reports_dir() / f"candle_book_dataset_m15_v1_report_{rs}.json"
    tp = reports_dir() / f"candle_book_dataset_m15_v1_report_{rs}.txt"
    ...
    lines = [
        "Book & Quality v2 - Candle Book Dataset M15 v1 Report",
        "=" * 74,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Candle Book Dataset M15 v1.")
...
def main() -> None:
    ...
    dcfg = cfg.get("candle_book_dataset_m15_v1", DEFAULT_CONFIG["candle_book_dataset_m15_v1"])
    source_coll = bq2["features_candle_book"]
    target_coll = bq2["dataset_candle_book"]
    ...
    report: Dict[str, Any] = {
        ...
        "run_id": f"candle_book_dataset_m15_v1_{rs}",
        "status": "running",
        "database": db_name,
        "symbol": symbol,
        ...
        "counts": {
            ...
            "skipped_missing_y": 0,
            "skipped_missing_book_direction": 0,
    ...
    print("=== Book & Quality v2 - Build Candle Book Dataset M15 v1 ===", flush=True)
    ...
    try:
        ...
        db[target_coll].create_index([("y.book_direction", ASCENDING), ("split", ASCENDING)], name="ix_y_book_direction_split")
        ...
        export_path = exports_dir() / f"candle_book_m15_v1_feature_names_{rs}.json"
        ...
        cursor = db[source_coll].find(query, projection={
            "_id": 0, "symbol": 1, "anchor_time": 1, "decision_time": 1,
            "entry_time": 1, "entry_price": 1, "features": 1, "feature_count": 1,
            "feature_meta": 1, "y": 1,
        }).sort("anchor_time", ASCENDING).batch_size(10000)
        ...
        for src in cursor:
            try:
                ...
                if book_direction is None:
                    report["counts"]["skipped_missing_book_direction"] += 1
                    continue
                ...
                y = {
                    ...
                    "label_strength": sf(y_full.get("label_strength", 0.0)),
                }
                doc = {
                    "symbol": src_symbol,
                    ...
                    "feature_version": FEATURE_VERSION,
                    "label_version": LABEL_VERSION,
                    "model_type": "candle_book_confirmation",
                    "anchor_timeframe": "M15",
                    "input_timeframes": ["M1", "M5", "M15"],
                    "split": split,
                    "x": x,
                ...
                report["label_distribution"][book_direction] = report["label_distribution"].get(book_direction, 0) + 1
