# -*- coding: utf-8 -*-
"""
Book & Quality v2 - Build MA Scenario Dataset M30 v1

Location:
    Book_Quality/04_dataset/ma_quality/02_build_ma_scenario_dataset_m30_v1.py

Purpose:
    Build the simplified MA Quality dataset after the architecture correction.

New rule:
    MA Quality Model predicts scenario + probability.
    API decides lot, TP/SL, trailing, exit, and operational risk.
...
Targets removed from training:
    quality_score
    risk_mode
    lot_multiplier
    tp/sl/trailing/exit modes

TEST ONLY:
    cd C:/Project/BookQuality/bq2/04_dataset/ma_quality
    python -u 02_build_ma_scenario_dataset_m30_v1.py --limit 1000 --reset

FULL BUILD:
    python -u 02_build_ma_scenario_dataset_m30_v1.py --reset
"""
...
SCRIPT_NAME = "02_build_ma_scenario_dataset_m30_v1.py"
...
def compact_scenario_y(y_full: Dict[str, Any]) -> Dict[str, Any]:
    return {k: y_full.get(k) for k in SCENARIO_TARGETS}
...
def save_reports(report: Dict[str, Any], rs: str) -> None:
    jp = reports_dir() / f"ma_scenario_dataset_m30_v1_report_{rs}.json"
    tp = reports_dir() / f"ma_scenario_dataset_m30_v1_report_{rs}.txt"
    ...
    lines = [
        "Book & Quality v2 - MA Scenario Dataset M30 v1 Report",
        "=" * 74,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build bq2 MA Scenario Dataset M30 v1.")
...
def main() -> None:
    ...
    dcfg = cfg.get("ma_scenario_dataset_m30_v1", DEFAULT_CONFIG["ma_scenario_dataset_m30_v1"])

    source_coll = bq2["features_ma_quality"]
    target_coll = bq2["dataset_ma_scenario"]
    ...
    report: Dict[str, Any] = {
        ...
        "run_id": f"ma_scenario_dataset_m30_v1_{rs}",
        "status": "running",
        "database": db_name,
        "symbol": symbol,
        ...
        "counts": {
            ...
            "skipped_missing_y": 0,
            "skipped_missing_scenario_target": 0,
        ...
        "feature_schema": {},
        "split_distribution": {},
        "target_distribution": {target: {} for target in SCENARIO_TARGETS},
        "warnings": [],
        "errors": [],
        "args": vars(args),
    }

    print("=== Book & Quality v2 - Build MA Scenario Dataset M30 v1 ===", flush=True)
    ...
    try:
        ...
        export_path = exports_dir() / f"ma_scenario_m30_v1_feature_names_{rs}.json"
        ...
        for src in cursor:
            try:
                ...
                if not isinstance(y_full, dict) or not y_full:
                    ...
                    continue
                if any(y_full.get(k) is None for k in SCENARIO_TARGETS):
                    report["counts"]["skipped_missing_scenario_target"] += 1
                    continue
                ...
                for target in SCENARIO_TARGETS:
                    val = y.get(target)
                    dist = report["target_distribution"][target]
