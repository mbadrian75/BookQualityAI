# -*- coding: utf-8 -*-
"""
Book & Quality v2 - Build Candle Book Side Features M15 v2

Location:
    Book_Quality/03_features/candle_book/03_build_candle_book_side_features_m15_v2.py

Purpose:
    Build no-leak features for the Candle Book Confirmation Model.

Model role:
    Candle Book confirms entry timing for each proposed side.
    It predicts side-specific outcomes later: buy_outcome / sell_outcome.
...
TEST:
    ...
    python -u 03_build_candle_book_side_features_m15_v2.py --limit 1000 --reset

FULL:
    python -u 03_build_candle_book_side_features_m15_v2.py --reset
"""
...
SCRIPT_NAME = "03_build_candle_book_side_features_m15_v2.py"
...
DEFAULT_CONFIG = {
    ...
    "candle_book_side_features_m15_v2": {
...
def load_config() -> Dict[str, Any]:
    ...
    cfg["bq2_collections"]["features_candle_book_side"] = "bq2_features_candle_book_side_m15_v2"
    return cfg
...
def build_features_for_label(label: Dict[str, Any], series: Dict[str, Series], fcfg: Dict[str, Any]) -> Tuple[Optional[Dict[str, float]], Dict[str, Any], Optional[str]]:
    ...
    for tf_name, idx, windows_key in [
        ...
        for w in fcfg.get(windows_key, DEFAULT_CONFIG["candle_book_side_features_m15_v2"][windows_key]):
...
def save_reports(report: Dict[str, Any], rs: str) -> None:
    jp = reports_dir() / f"candle_book_side_features_m15_v2_report_{rs}.json"
    tp = reports_dir() / f"candle_book_side_features_m15_v2_report_{rs}.txt"
    ...
    lines = [
        "Book & Quality v2 - Candle Book Side Features M15 v2 Report",
        "=" * 74,
    ...
    for k, v in report.get("buy_outcome_distribution", {}).items():
        lines.append(f"{k:<34}: {v}")

    lines += ["", "Sell Outcome Distribution", "-" * 74]
    for k, v in report.get("sell_outcome_distribution", {}).items():
        lines.append(f"{k:<34}: {v}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Candle Book Side Features M15 v2.")
...
def main() -> None:
    ...
    fcfg = cfg.get("candle_book_side_features_m15_v2", DEFAULT_CONFIG["candle_book_side_features_m15_v2"])

    label_coll = bq2.get("labels_candle_book_side") or bq2.get("labels_candle_book") or "bq2_labels_candle_book_side_m15_v2"
    target_coll = bq2.get("features_candle_book_side") or bq2.get("features_candle_book") or "bq2_features_candle_book_side_m15_v2"

    report: Dict[str, Any] = {
        "script_name": SCRIPT_NAME,
        "run_id": f"candle_book_side_features_m15_v2_{rs}",
        "status": "running",
        "database": db_name,
        "symbol": symbol,
    ...
    print("=== Book & Quality v2 - Build Candle Book Side Features M15 v2 ===", flush=True)
    ...
    try:
        ...
        db[target_coll].create_index([("y.buy_outcome", ASCENDING), ("anchor_time", ASCENDING)], name="ix_buy_outcome_anchor_time")
        db[target_coll].create_index([("y.sell_outcome", ASCENDING), ("anchor_time", ASCENDING)], name="ix_sell_outcome_anchor_time")
        ...
        for label in cursor:
            try:
                ...
                y = label.get("y")
                if not isinstance(y, dict) or not y.get("buy_outcome") or not y.get("sell_outcome"):
                ...
                direction = y.get("book_direction_v2", "NOTRADE")
                ...
                report.setdefault("buy_outcome_distribution", {})[buy_outcome] = report.setdefault("buy_outcome_distribution", {}).get(buy_outcome, 0) + 1
                report.setdefault("sell_outcome_distribution", {})[sell_outcome] = report.setdefault("sell_outcome_distribution", {}).get(sell_outcome, 0) + 1
                ...
                doc = {
                    ...
                    "y": {
                        "book_direction_v2": direction,
                        "buy_outcome": buy_outcome,
                        "sell_outcome": sell_outcome,
                        "buy_method": y.get("buy_method"),
                        "sell_method": y.get("sell_method"),
                        "buy_confidence_label": sf(y.get("buy_confidence_label", 0.0)),
                        "sell_confidence_label": sf(y.get("sell_confidence_label", 0.0)),
                    },
                    "created_at": now,
                    "updated_at": now,
                }
