# -*- coding: utf-8 -*-
"""
Book & Quality v2 - Build Candle Book Features M15 v1

Location:
    Book_Quality/03_features/candle_book/01_build_candle_book_features_m15_v1.py
...
Model role:
    ...
    It predicts book_direction = BUY / SELL / NOTRADE.
...
TEST:
    ...
    python -u 01_build_candle_book_features_m15_v1.py --limit 1000 --reset

FULL:
    python -u 01_build_candle_book_features_m15_v1.py --reset
"""
...
SCRIPT_NAME = "01_build_candle_book_features_m15_v1.py"
...
def build_features_for_label(label: Dict[str, Any], series: Dict[str, Series], fcfg: Dict[str, Any]) -> Tuple[Optional[Dict[str, float]], Dict[str, Any], Optional[str]]:
    ...
    for tf_name, idx, windows_key in [
        ...
        for w in fcfg.get(windows_key, DEFAULT_CONFIG["candle_book_features_m15_v1"][windows_key]):
...
def save_reports(report: Dict[str, Any], rs: str) -> None:
    jp = reports_dir() / f"candle_book_features_m15_v1_report_{rs}.json"
    tp = reports_dir() / f"candle_book_features_m15_v1_report_{rs}.txt"
    ...
    lines = [
        "Book & Quality v2 - Candle Book Features M15 v1 Report",
        "=" * 74,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Candle Book Features M15 v1.")
...
def main() -> None:
    ...
    fcfg = cfg.get("candle_book_features_m15_v1", DEFAULT_CONFIG["candle_book_features_m15_v1"])

    label_coll = bq2["labels_candle_book"]
    target_coll = bq2["features_candle_book"]

    report: Dict[str, Any] = {
        "script_name": SCRIPT_NAME,
        "run_id": f"candle_book_features_m15_v1_{rs}",
        "status": "running",
        "database": db_name,
        "symbol": symbol,
    ...
    print("=== Book & Quality v2 - Build Candle Book Features M15 v1 ===", flush=True)
            ...
            try:
                ...
                doc = {
                    ...
                    "y": {
                        ...
                        "label_strength": sf(y.get("label_strength", 0.0)),
                    },
                    "created_at": now,
                    "updated_at": now,
                }
