...
Purpose:
- Validate MA context feature collection after build
...
RUN_TYPE = "sce_validate_ma_context_features"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate SCE MA context features.")
    parser.add_argument("--mongo-uri", default=MONGO_URI)
    parser.add_argument("--database", default=DB_NAME)
    parser.add_argument("--collection", default=COLL_FEATURES_MA_CONTEXT_M5_V1)
...
def write_report(args: argparse.Namespace, status: str, stats: Dict[str, Any], error_text: str = "") -> Path:
    ...
    lines: List[str] = []
    lines.append("Side Context Entry AI - MA Context Feature QA Report")
    lines.append("=" * 72)
    ...
    lines.append("")
    lines.append("Alignment distributions:")
    for tf in [x.lower() for x in MA_CONTEXT_TFS]:
        lines.append(f"{tf}_alignment:")
        for key, value in stats.get(f"{tf}_alignment", {}).items():
            lines.append(f"  {key:28}: {value} | {pct(value, total)}")
    lines.append("")
    lines.append("Trend phase distributions:")
    for tf in [x.lower() for x in MA_CONTEXT_TFS]:
        lines.append(f"{tf}_trend_phase:")
        for key, value in stats.get(f"{tf}_trend_phase", {}).items():
            lines.append(f"  {key:28}: {value} | {pct(value, total)}")
    lines.append("")
    lines.append("Context totals:")
    for field in ["ma_context_side_alignment_total", "ma_context_side_slope_total", "ma_context_all_aligned"]:
...
def main() -> int:
    ...
    try:
        ...
        for tf in [x.lower() for x in MA_CONTEXT_TFS]:
            stats[f"{tf}_alignment"] = kv_counts(coll, f"{tf}_alignment", query)
            stats[f"{tf}_trend_phase"] = kv_counts(coll, f"{tf}_trend_phase", query)
        for field in ["ma_context_side_alignment_total", "ma_context_side_slope_total", "ma_context_all_aligned"]:
            stats[field] = kv_counts(coll, field, query)
        report_path = write_report(args, "success", stats)
        print("SCE MA context feature QA completed.")
        print(f"Report: {report_path}")
        return 0
    except Exception:
        error_text = traceback.format_exc()
        print("ERROR: SCE MA context feature QA failed.")
        print(error_text)
